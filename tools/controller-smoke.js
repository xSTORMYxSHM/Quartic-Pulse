...
assert(window.QuarticBackgroundRenderPolicy?.evaluate, 'Background render policy module did not load.');
const hiddenObsPolicy = window.QuarticBackgroundRenderPolicy.evaluate({ backgrounded: true, obsOutputOpen: true, obsSyncFps: 60 });
...
const obsThirtyPolicy = window.QuarticBackgroundRenderPolicy.evaluate({ isObsOutput: true, obsSyncFps: 30 });
assert(obsThirtyPolicy.drawVisual && !obsThirtyPolicy.updateInterface && obsThirtyPolicy.targetFps === 30,
  'The OBS renderer must honor the selected 30 FPS efficiency rate.');
